...
(function () {
    ...
    function buildTreeByCommunities(dataObj, showNotes) {
        ...
        if (dataObj.useWeights) {
            ...
            if (!W) {
                W = 1e-7;
            } //  This avoids some errors with disconnected components
            var inv2m = 1 / (2 * W);
        } else {
            for (var hash in dataObj.distances) {
                var keys = hash.split("~");
        ...
        for (var hash in dataObj.distances) {
            ...
            if (dataObj.useWeights) {
                deltaQ[hash] =
                    2.0 * inv2m * dataObj.distances[hash] -
                    2.0 * inv2m * inv2m * k[i] * k[j];
            } else {
        ...
        binaryTreeWalk(
            ...
            tree,
            0,
            function (currentNode, tree, depth) {
    ...
    function findSubCommunities(treeObj, depth, prevGroup) {
        ...
        for (var hash in treeObj.distances) {
            ...
            if (group[i] == group[j]) {
                ...
                dataObjList[group[i]].distances[newHash] =
                    treeObj.distances[hash];
            }
        }
            ...
            for (var g = 0; g < numGroups; g++) {
                ...
                if (innerTreeObj && innerTreeObj.tree) {
                    ...
                    if (innerGroups > 1) {
                        ...
                        var subgroups = findSubCommunities(
                            innerTreeObj,
                            depth + 1,
                            g
                        );
    ...
    netClustering.cluster = function (
        nodes,
        edges,
        clusterAttr,
        edgesCountAttr
        ...
        if (edges.length > 0 && typeof edges[0].source === "object") {
            ...
            edges.forEach(function (d) {
                linksForClustering.push({
                    ...
                    target: d.target,
                    count:
                        d[edgesCountAttr] !== undefined ? d[edgesCountAttr] : 1,
                });
            });
        }
        ...
        linksForClustering.forEach(function (d) {
            var hash =
                Math.min(d.source, d.target) +
                "~" +
                Math.max(d.source, d.target); // hash always has left key < right key})
